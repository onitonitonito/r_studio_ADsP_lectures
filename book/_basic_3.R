...
rm(list = ls())            # 모든 변수를 삭제 (이니셜라니즈)
ls()


.kkk <- 12                 # 히든변수는 환경변수 리스트에 보이지 않는다.
.kkk
objects()                  # 변수를 조회하는 함수
objects(all.names=TRUE)    # 히든변수이므로 이런 변수까지 조회하는 함수
...
# 객체의 종류를 알고싶을땐 --> mode(), class()를 사용한다.
mode(a)
class(a)
...
mean(df)      # 워닝! = NA반환 - 인자가 (수치형/논리형) 아니므로 NA를 반환.
...
z == z1       # 결과값 = F T T
any(z == z1)  # 벡터의 값들 중 하나라도 같은가?  --> or(TRUE)?
...
# 벡터(리스트)는 포지션 넘버로 불러낸다 (**첫 포지션=1 부터 카운트 한다, 0부터 아님.)
a[1]
a[2]
a[3]
...
# 인덱스가 (-)인 경우는 그(범위)의 값은 제외 된다는 의미
a[-1]           # 첫번째를 제외한 모두
a[-(2:3)]       # 2~3을 제외한 모두
a[-c(1,3,5,7)]  # 음수는 '제외한 모두'를 의미한다.


# 중앙값(median)보다 큰 모든 원소를 선택 할 경우.
# 중앙값(median) = 일열로 크기별로 세웠을때, 중앙의 원소 (NA값을 처리해줘야 함)
#  __ 홀수 일경우는 센터값이 있다
...
median(a)             # 짝수(8)이므로 4, 4+1 위치값을 평균 = (2+3)/2 = 2.5

a > median(a)         # F F F F T T T T
a[a>median(a)]        # 조건문은 [] 를 사용한다.

# 상하위 5% 안에 있는 모든 원소를 선택 (or)
...
a <- c(1,2,NA,3,4)
a
mean(a)               # 계산이 되지 않는다--- NA가 나온다

b<-c(1,2,NULL,3,4)
b
mean(b)               # NULL 은 인식하지 않는다. --- 맴버 4개로 인식 mean : 10/4 = 2.5
...
# 각 맴버마다 '컬럼' 이름을 부여할 수 있다 -- 포지션번호 대신 커럼이름으로 '호출' 가능하다.
names(a) <- c("1st","2nd","3th","4th","5th","6th","7th","8th")    # 갯수가 서로 같아야 한다.
a
